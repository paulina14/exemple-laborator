...
namespace Lab1.Domain.Models
{
    ...
    public static partial class ShoppingCart
    {
        ...
        public record FailedCart : IShoppingCart
        {
            internal FailedCart(IReadOnlyCollection<UnvalidatedProductsCart> productList, Exception exception)
            {
                ProductList = productList;
                Exception = exception;
            }
            public IReadOnlyCollection<UnvalidatedProductsCart> ProductList;
       ...
       /* public record PaidHeader : IShoppingCart
        {
            internal PaidHeader(IReadOnlyCollection<CalculetedHeader> hearderList, string csv, DateTime pay)
            {
                HeaderList = hearderList;
                Pay = pay;
                Csv = csv;
            }
            public IReadOnlyCollection<CalculetedHeader> HeaderList { get; }
            public DateTime Pay { get; }
            public string Csv { get; }
        }*/
